...
# Ex. 4: Let's have yet another at lengthy_word. Can you write code
...
for i, char in enumerate(lengthy_word):
	if i == (len(lengthy_word)-1):
		char_dict["Last word!"] = 1
	else:
		next_char = lengthy_word[i+1]
...
# or alternatively, using pass, instead of raise:
lengthy_word = "thisisthelongestwordontheplanet"
char_dict = {}
for i, char in enumerate(lengthy_word):
	if i == (len(lengthy_word)-1):
...
# Ex. 7: Remember the exercise in cryptography and ROT13 from the previous
# chapter? This alternative name comes from Julius Caesar, who used it to
# communicate with his generals. ROT13 ("rotate by 13 places") is a widely
# used example of a Caesar cipher. The shift here is +13. Can you write a
# code block that creates a dictionary which for each letter in the
# (lowercase) roman alphabet contain the encrypted version according
# to ROT13? Next, use this dictionary to decode the following sentence
...
# "Pnrfne pvcure? V zhpu cersre Pnrfne fnynq!" Use the pass statement
# which you learned about in a previous excercise to make that you
# don't try to decode punctuation marks!
...
# Ex. 8: The third person singular verb form in English is
